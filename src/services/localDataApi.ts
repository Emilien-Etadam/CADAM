import { getLocalBackendBaseUrl } from '@/lib/localBackend';
...
export async function apiGetConversation(conversationId: string): Promise<Conversation> {
...
export async function apiListMessages(conversationId: string): Promise<Message[]> {
...
export async function apiListRecentConversations(limit: number): Promise<Conversation[]> {
...
export async function apiPatchConversation(
  id: string,
  patch: Partial<Pick<Conversation, 'title' | 'privacy' | 'type' | 'current_message_leaf_id' | 'settings'>>,
): Promise<Conversation> {
...
export async function apiVisualAssistantMessages(
  ...
  return j(`/api/visual-messages?conversationId=${encodeURIComponent(conversationId)}`);
}
